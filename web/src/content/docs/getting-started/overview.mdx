...
import { PinInput } from '../../../Examples/React/PinInput/PinInput.tsx'
...
<Button className={button({ intent:"secondary", state:"ghost" })}>
Button
</Button>
</div>
```tsx
...
<Button className={button({ intent:"secondary", state:"ghost" })}>Button</Button>;
```
...
Assembled components built from the Base components, such as the pin-input above are distributed via VScode snippets. This allows you to rapidly build complex systems with just a few keystrokes. Have a look at the [VsCode Docs](/docs/getting-started/vsc#snippets) for how to use.